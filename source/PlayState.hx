	...
	override public function create()
	{
	  ...
	  case 'senpai' | 'roses':
		...
		case 'noche' | 'desierto':
			...
			var mountains:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('fiesta/mountain'));
			mountains.antialiasing = true;
			mountains.scrollFactor.set(0.2, 0.2);
			mountains.active = false;
			add(mountains);

			var bgDirt:FlxSprite = new FlxSprite(-600, -150).loadGraphic(Paths.image('fiesta/bgDirt'));
			bgDirt.setGraphicSize(Std.int(bgDirt.width * 1));
			bgDirt.antialiasing = true;
			bgDirt.screenCenter(X);
			bgDirt.scrollFactor.set(0.3, 0.3);
			bgDirt.active = false;
			add(bgDirt);

			bopFellas = new FlxSprite(-480, -300);
			bopFellas.frames = Paths.getSparrowAtlas('fiesta/bopFellas');
			bopFellas.animation.addByPrefix('bop', "bop", 24, false);
			bopFellas.antialiasing = true;
			bopFellas.scrollFactor.set(0.33, 0.33);
			bopFellas.setGraphicSize(Std.int(bopFellas.width * 0.65));
			bopFellas.screenCenter(XY);
			bopFellas.x += bopFellas.width/4 -150;
			bopFellas.y += 100;
			bopFellas.updateHitbox();
			add(bopFellas);

			var filter:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('fiesta/light filter'));
			filter.scale.set(6, 6);
			filter.alpha=0.25;
			add(filter);

			var fg:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('fiesta/fg'));
		...
		case 'globetrotter':
			...
			var bg:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('rooftop/sky'));
			bg.screenCenter(XY);
			bg.antialiasing = true;
			...
			behind.scrollFactor.set(1, 1);
			behind.active = false;
			behind.setGraphicSize (Std.int(behind.width * 1.3));
			behind.x -= 525;
			behind.y -= 75;
			add(behind);

			bopIt = new FlxSprite(0, -300);
			bopIt.frames = Paths.getSparrowAtlas('rooftop/bopIt');
			bopIt.animation.addByPrefix('bopIt', "bopIt", 24, false);
			bopIt.antialiasing = true;
			bopIt.scrollFactor.set(1, 1);
			bopIt.setGraphicSize (Std.int(bopIt.width * 0.7));
			bopIt.x -= 350;
			bopIt.updateHitbox();
			add(bopIt);

			var wall:FlxSprite = new FlxSprite(0, -300).loadGraphic(Paths.image('rooftop/brick'));
	...
	override public function update(elapsed:Float)
	{
		...
		if(curSong == "Pain-Gran-Venta"){
		 	if(curStep==2520 && doneTransShit==false){
					...
					var stuf = new TransitionData(FADE, FlxColor.BLACK, 0.7, new FlxPoint(0, 1),
						{asset: diamond, width: 32, height: 32}, new FlxRect(-200, -200, FlxG.width * 1.4, FlxG.height * 1.4));
					fakeTransition = new TransitionMod(stuf);
		...
		if ((upP || rightP || downP || leftP) && generatedMusic)
			{
				...
				if(perfectMode){
					...
					for (idx in 0...controlArray.length){
						...
						if(pressed && ignoreList.contains(idx)==false )
							badNoteCheck();
					}
